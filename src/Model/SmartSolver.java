...
public class SmartSolver {
    ...
    private int maxRepitedTilePoints = 0;
    private Tile repitedTile = null;
    private boolean isRepitedTileSolution = false;

    public SmartSolver(Board board) {
    ...
    public ArrayList<Tile> solve(ArrayList<Tile> hand) {
        for (int p = 0; p < hand.size(); p++) {
            for (Tile tile : hand) {
                if (this.repitedTile == null && !tile.equals(hand.get(p)) && (hand.get(p).getColor().equals(tile.getColor()) && (hand.get(p).getShape().equals(tile.getShape())))) {
                    System.out.println(tile.toString());
                    this.repitedTile = tile;
                }
            }
        }
//        System.out.println("repited tile: " + this.repitedTile.toString());
        ...
        if (k == hand.size() - 1) {
            if (j == hand.size() - 1) {
                if (origen != null && this.filterSolution(solution, board, origen) > 0) {
                    ...
                    } else if (this.isRepitedTileSolution && temPoints > this.maxRepitedTilePoints) {
                        this.solutions.clear();
                        this.solutions.add(0, new ArrayList<>(solution));
                        this.maxRepitedTilePoints = temPoints;

                    } else if (!this.isRepitedTileSolution && temPoints > this.maxPoints) {
...
//                        System.out.println("se fue 3\n");
        ...
        if (this.repitedTile != null && this.repitedTile.getShape().equals(solution.get(0).getShape()) && this.repitedTile.getColor().equals(solution.get(0).getColor())) {
            isRepitedTileSolution = true;
        }
...
//                    System.out.println("se fue 5\n");
                ...
                if (this.repitedTile != null && this.repitedTile.getShape().equals(solution.get(i).getShape()) && this.repitedTile.getColor().equals(solution.get(i).getColor())) {
                    isRepitedTileSolution = true;
                }
            }
        }
...
//    private ArrayList<ArrayList<Tile>> getPossibleQwirkle(ArrayList<Tile> board) {
//        ArrayList<ArrayList<Tile>> possibleQwirkle = new ArrayList<>();
